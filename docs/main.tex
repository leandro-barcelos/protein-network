...
# JSON: Estrutura do arquivo de anotações
{
  ...
  "Hemoglobin, beta-type": ["B", "D"]
	...
	A Tabela~\ref{tab_topologia} resume as métricas da rede C$\alpha$ a 8 Å (valor adotado na literatura) para o alvo (6B1T) e para uma proteína de teste (1AON).
	...
	As duas redes têm o mesmo perfil. Com o limiar de 8 Å, toda a estrutura forma um único componente conexo, com grau médio próximo de 11 e distribuição de graus em forma de sino (Figuras~\ref{fig_deg_6b1t} e~\ref{fig_deg_1aon}). Como, para esse tipo de rede, o grau de um nó é o número de vizinhos espaciais dentro do limiar, um grau maior indica que a região que o átomo está é mais densas, enquanto um grau menor indica uma região mais expostas ou nas extremidades das cadeias. O alto clustering reflete o fato de que os vizinhos de um nó tendem a ser vizinhos entre si, e a assortatividade positiva indica que nós de grau alto se conectam preferencialmente a outros nós de grau alto. A centralidade de intermediação destaca os nós situados nos caminhos entre as regiões densas do grafo (Figura~\ref{fig_betw_6b1t}), tipicamente nos pontos em que cadeias diferentes se tocam.
	...
	Como cada componente isolado já representa uma família, todos os algoritmos convergem para a mesma partição, recuperando os oito componentes como oito comunidades. A partição identifica exatamente as famílias anotadas, como pode ser confirmado pela matriz de contingência entre comunidades e famílias (Tabela~\ref{tab_contingencia}). Por ser uma matriz diagonal, nenhuma comunidade mistura famílias, ou seja, cada comunidade corresponde exatamente a uma família.
